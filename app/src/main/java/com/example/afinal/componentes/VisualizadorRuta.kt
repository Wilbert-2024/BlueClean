                ...
                Column(Modifier.padding(24.dp)) {
                    ...
                    LazyColumn(modifier = Modifier.fillMaxWidth().heightIn(max = 400.dp)) {
                        itemsIndexed(puntos) { index, punto ->
                            FilaLineaTiempo(punto = punto, esPrimero = index == 0, esUltimo = index == puntos.size - 1)
                        }
                    }

                    TextButton(onClick = onDismiss, modifier = Modifier.align(Alignment.End).padding(top = 16.dp)) {
                        Text("CERRAR", color = Colores.VerdePrincipal, fontWeight = FontWeight.Bold)
                    }
                }
            }
        }
...
fun FilaLineaTiempo(punto: PuntoRecorrido, esPrimero: Boolean, esUltimo: Boolean) {
    Row(modifier = Modifier.fillMaxWidth().height(IntrinsicSize.Min)) {
            ...
            if (!esUltimo) {
                Canvas(Modifier.weight(1f).width(2.dp)) {
                    drawLine(
                        ...
                        start = androidx.compose.ui.geometry.Offset(0f, 0f), end = androidx.compose.ui.geometry.Offset(0f, size.height), strokeWidth = 4f,
                        pathEffect = if (punto.estado != EstadoPuntoRecorrido.COMPLETADO) PathEffect.dashPathEffect(floatArrayOf(10f, 10f), 0f) else null
                    )
                }
            }
        }
        ...
        Column(modifier = Modifier.weight(1f).padding(bottom = 24.dp)) {
            Text(punto.nombre, fontSize = 16.sp, fontWeight = if (punto.estado == EstadoPuntoRecorrido.ACTUAL) FontWeight.ExtraBold else FontWeight.Bold, color = if (punto.estado == EstadoPuntoRecorrido.ACTUAL) Colores.VerdePrincipal else Colores.TextoOscuro)
            val subtitulo = when {
            ...
            if (subtitulo.isNotEmpty()) Text(subtitulo, fontSize = 13.sp, color = if(punto.estado == EstadoPuntoRecorrido.ACTUAL) Colores.VerdePrincipal else Colores.TextoGris)
            if (!esUltimo) { Spacer(Modifier.height(12.dp)); HorizontalDivider(thickness = 0.5.dp, color = Colores.GrisSeparador.copy(0.5f)) }
        }
    }
}